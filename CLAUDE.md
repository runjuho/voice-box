...
### 결과물 전달 방식 (2026-09-23 확정 · 사용자 아이폰 카톡에서 동작 확인 — 이 방식만 쓴다)
...
### 카드 구성 규칙 (2026-09-23 사용자 피드백)

**1) 맨 앞장(커버)에 "내 의견·코멘트가 들어있다"는 걸 크게 알린다.**
- 정보 정리만 있는 게 아니라 런주호 개인 판단이 들어있다는 걸 커버에서 바로 알 수 있어야 한다.
- 가시성·가독성 우선: 작은 각주가 아니라 커버 안에서 눈에 띄는 요소로 (예: 포인트 컬러 캡슐 배지 "런주호 코멘트 포함", 또는 "살지 말지 · 제 의견은 N장"처럼 몇 번째 장인지까지).
- 애플 톤 유지 — 배지는 캡슐(999px), 액센트 블루 배경이면 흰 글씨.

**2) 장비 소식에는 "바이어 가이드"(내 코멘트) 장을 넣는다.** 형식은 2026-09-23 아디오스 프로 5 8번 카드 기준:
- 맨 위 "결론부터" 블록에 추천/비추천을 한 줄로 (예: "프로4 현역·마일리지 넉넉하면, 지금은 구매 비추천").
- 근거는 해외 실사용 리뷰(레딧 우선, 없으면 착용 리뷰 매체)에서 찾은 **단점** 위주로 3~4개, 한 줄씩.
- 마지막에 "이런 사람이면 사도 된다"는 반대 조건을 한 줄로.
- 출처를 정직하게: 레딧 스레드가 없으면 "레딧"이라고 쓰지 말고 "해외 착용 리뷰 매체 종합"이라고 쓴다 (발매 직후엔 레딧 후기가 거의 없다).
- 카드 한 장 안에 푸터까지 다 들어가게 문장을 줄인다 — 렌더 후 넘침을 꼭 확인.
